...
**0.1.4 fixes the weather services not loading.** 0.1.3 launches, and most of it works: NWS alerts, the precipitation panel and the warning notifications all came through. What failed was everything from Open-Meteo — the extended forecast, upper-air winds for the jet stream map, precipitation types, and place search — with `the archive entry was compressed using an unsupported compression method`. Not a connectivity problem: the bytes arrived, the platform just could not expand them. Responses are compressed with one negotiated encoding, and `deflate` is ambiguous on the wire — some servers wrap it in a zlib header, some send it raw, and inflaters disagree about which to expect. The app now asks only for gzip, and if a host's compressed reply still cannot be read it re-fetches from that host uncompressed and remembers to keep doing so.

**Tap the status line for details.** That one orange line has room for a single truncated sentence, which was not enough to diagnose anything. Tapping it now opens every warning in full plus a log of each request this session made — host, result, size, timing and the underlying exception — with Share and Copy. If something is still wrong, that report is the thing to send.

A note on honesty: the Open-Meteo fix has still never been tested against the live service, for the same reason as everything else here. The reasoning is sound and the fallback should work whatever the actual cause, but it is reasoning, not evidence.
