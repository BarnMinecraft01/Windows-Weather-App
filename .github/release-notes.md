...
**0.1.3 fixes the Android launch crash.** A bug report from the Pixel Fold that kept failing settled it, and it was two faults stacked on top of each other.

The first was the Release build being trimmed and ahead-of-time compiled — both are on by default for .NET Android and both are hostile to code reached only by reflection. The runtime aborted the process outright (`klass->instance_size == instance_size` in Mono's class loader), which is why nothing the app could catch ever ran and why no error screen appeared. Turning both off in 0.1.2 fixed it: the crash does not appear again anywhere in the report after that version was installed.

The second was the activity theme. Avalonia's activity is an androidx `AppCompatActivity`, and AppCompat refuses to build its content view under a theme that does not descend from `Theme.AppCompat` — this one inherited a platform theme instead. It threw *after* the activity reached the screen, so the app opened and closed. With 0.1.2's crash reporting in place the phone recorded the exception by name, which is what made it findable. CI now fails the build if that theme is ever wrong again.
